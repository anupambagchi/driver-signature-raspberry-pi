...
# THIS PROGRAM CONTAINS THE PROTOCOL TO BE CHANGED
...
from obd_sensors import hex_to_int
 ...
 # This number must be changed such that it is compatible with car.
PROTOCOL = "6"
...
class OBDPort:
     ...
     def __init__(self,portnum,_notify_window,SERTIMEOUT,RECONNATTEMPTS):
         ...
         self.send_command("AT SP " + PROTOCOL)  # telling the program to use 9141-2 protocol for Asian cars. Ford's 'ISO 15765-4 CAN' protocol is 6
     ...
     def get_dtc(self):
          ...
          outfile.write("Number of stored DTC:" + str(dtcNumber) + " MIL: " + str(mil) + '\n')
          ...
          for i in range(0, ((int(dtcNumber)+2)/3)):
            ...
            for i in range(0, 3):
                ...
                val2 = hex_to_int(ressegment[6+i*6:8+i*6]) #get DTC codes from response (3 DTC each 2 bytes)
