...
def hex_to_int(str):
    if len(str) == 0 or str == 'RROR' or str == 'RR' or str[len(str) - 1] == ':' or str == 'OR':
        return 0
    # print 'hex_to_int(' + str + ')\n'
    i = eval("0x" + str, {}, {})
    return i
...
# Returns an array of two numbers from a result containing two bytes
def decrypt_AB(code):
    # first byte is byte after PID and without spaces
    res = []
...
SENSORS = [
    ...
    Sensor("abs_load"              , "Absolute Load Value"			, "01431", percent_scale    ,""       ),
    Sensor("rel_throttle_pos"      , "Relative Throttle Position"	, "0145",  percent_scale    ,""       ),
    Sensor("ambient_air_temp"      , "Ambient Air Temperature"	    , "0146",  temp             ,"F"      ),
    Sensor("abs_throttle_pos_b"    , "Absolute Throttle Position B"	, "0147",  percent_scale    ,""       ),
    Sensor("acc_pedal_pos_d"       , "Accelerator Pedal Position D"	, "0149",  percent_scale    ,""       ),
    Sensor("acc_pedal_pos_e"       , "Accelerator Pedal Position E"	, "014A",  percent_scale    ,""       ),
    Sensor("comm_throttle_ac"      , "Commanded Throttle Actuator"	, "014C",  percent_scale    ,""       ),
    Sensor("rel_acc_pedal_pos"     , "Relative Acc Pedal Pos"       , "015A",  percent_scale    ,""      ),
    Sensor("eng_fuel_rate"         , "Engine Fuel Rate"             , "015E",  frate            ,"L/h"   ),
    Sensor("drv_demand_eng_torq"   , "Drivers Demand Engine Torque" , "0161",  torque_percent   ,"%"   ),
    Sensor("act_eng_torq"          , "Actual Engine Torque"         , "0162",  torque_percent   ,"%"   ),
    Sensor("eng_ref_torq"          , "Engine Reference Torque"      , "0163",  torque_value     ,"Nm"   ),
    ]
